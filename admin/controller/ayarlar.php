    ...
    if( $user["access"]["ayarlar"] != 1  ):
      ...
      if( $_POST ):
          ...
          if ( $_FILES["logo2"] && ( $_FILES["logo2"]["type"] == "image/jpeg" || $_FILES["logo2"]["type"] == "image/jpg" || $_FILES["logo2"]["type"] == "image/png" || $_FILES["logo2"]["type"] == "image/gif"  ) ):
            $logo_name      = $_FILES["logo2"]["name"];
            $uzanti         = substr($logo_name,-4,4);
            $md5            = md5(rand(1,999999));
            $logo_newname2   = "assets/cloud/uploads/$md5.png";
            $upload_logo    = move_uploaded_file($_FILES["logo2"]["tmp_name"],$logo_newname2);
          elseif( $settings["site_logo"] != "" ):
            $logo_newname2   = $settings["site_logo2"];
          else:
            $logo_newname2   = "";
          endif;
          ...
          elseif( $settings["favicon"] != "" ):
            ...
            $update = $conn->prepare("UPDATE settings SET instagram=:instagram, facebook=:facebook, twitter=:twitter, linkedin=:linkedin, site_currency=:site_currency, site_mail=:site_mail, site_telephone=:site_telephone, site_address=:site_address, site_maps=:site_maps, site_maintenance=:site_maintenance, site_name=:name, site_logo=:logo, site_logo2=:logo2, favicon=:fv, recaptcha=:recaptcha, recaptcha_key=:recaptcha_key, recaptcha_secret=:recaptcha_secret, custom_header=:custom_header, custom_footer=:custom_footer WHERE id=:id ");
            $update->execute(array("id"=>1,"instagram"=>$instagram,"facebook"=>$facebook,"twitter"=>$twitter,"linkedin"=>$linkedin,"site_currency"=>$site_currency,"site_mail"=>$site_mail,"site_telephone"=>$site_telephone,"site_address"=>$site_address,"site_maps"=>$site_maps,"site_maintenance"=>$site_maintenance,"name"=>$name,"logo"=>$logo_newname,"logo2"=>$logo_newname2,"fv"=>$fv_newname,"recaptcha"=>$recaptcha,"recaptcha_secret"=>$recaptcha_secret,"recaptcha_key"=>$recaptcha_key,"custom_footer"=>$custom_footer,"custom_header"=>$custom_header ));
